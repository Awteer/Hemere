...
def generate_pareto_dataset():
    ...
    diameters = np.arange(60.0, 130.0, 0.5)
    
    # 積層構成の総当たり
    ply_2_opts = range(0, 6) 
    ply_conc_opts = range(0, 5) 
    ...
    for R in diameters:
        for p2 in ply_2_opts:
            for p_conc in ply_conc_opts:
                ...
                # 集中積層の適用
                if p_conc > 0:
                     current_plies[4] = p_conc
                     current_plies[7] = p_conc
    ...
    # 対数スケールでビンを切り、各剛性ランクにおける最小重量を取得
    n_bins = 2000 
...
class CriticalVerifier:
    def __init__(self, EI, R, W_actual, W_pred):
        ...
        self.W_pred = W_pred
        # ゼロ除算回避のための小さなepsilon
        self.rel_error = (self.W_pred - self.W_actual) / (self.W_actual + 1e-9) * 100.0
        
    def plot_dashboard(self):
        fig = plt.figure(figsize=(18, 10))
        fig.suptitle("Critical Verification: AI Surrogate Model (Neural Network)", fontsize=16)
        ...
        ax1.set_title('Accuracy Check (Actual vs Pred)')
        ...
        # Plot 3: 3D Visualization of Pareto Surface
        ...
        p = ax3.scatter(np.log10(self.EI[idx_sub]), self.R[idx_sub], self.W_actual[idx_sub], 
                        c=self.rel_error[idx_sub], cmap='bwr', s=5, vmin=-2, vmax=2)
        ax3.set_xlabel('Log10(EI)')
        ax3.set_ylabel('Diameter R')
        ax3.set_zlabel('Weight')
        ax3.set_title('3D Pareto Surface & Error')
        fig.colorbar(p, ax=ax3, label='Error %', shrink=0.6)

        # Plot 4: Bias vs Diameter
        ax4 = fig.add_subplot(2, 3, 4)
        sc4 = ax4.scatter(self.R, self.rel_error, c=np.log10(self.EI), cmap='viridis', s=5, alpha=0.6)
        ...
        # Plot 5: Bias vs Stiffness
        ax5 = fig.add_subplot(2, 3, 5)
        sc5 = ax5.scatter(self.EI, self.rel_error, c=self.R, cmap='plasma', s=5, alpha=0.6)
...
def run_ai_simulation_rf():
    ...
    # --- 特徴量エンジニアリング ---
    # Random Forestでも、対数変換は「広いダイナミックレンジ」を扱うのに有効です
    log_EI = np.log10(EI_best)
    X = np.column_stack((log_EI, R_best))
    y = W_best

    # --- 【変更点】モデルをRandom Forestに変更 ---
    # 決定木ベースなので、スケーリング(StandardScaler)は必須ではありませんが、
    # パイプラインの形を維持して実装します。
    model_pipeline = Pipeline([
        # ('scaler', StandardScaler()), # 木モデルにはスケーリング不要（あっても害はない）
        ('rf', RandomForestRegressor(
            n_estimators=200,      # 木の数（多いほど滑らかになる）
            max_depth=None,        # 木の深さ（制限なしで複雑な階段に追従させる）
            min_samples_leaf=1,    # 1データでも分離する（過学習気味に攻める）
            random_state=42,
            n_jobs=-1              # 全CPUコアを使って高速計算
        ))
    ])

    print("-" * 50)
    print("Training Random Forest Surrogate Model...")
    print("Strategy: Tree-based model to capture discrete ply steps")
    ...
    # --- モデル保存 ---
    model_filename = 'spar_rf_model.pkl'
